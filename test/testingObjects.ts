...
export const testHeadingsWithSpecialChars: HeadingCache[] = [
    ...
    {
        heading:
            'Title 1 level 3 [[wikilink1]] [[wikilink2|wikitext2]] [mdlink](https://mdurl)',
        level: 3,
        position: defaultPosition
    },
    {
...
export const initialStandardHeadings = Object.values(testStandardHeadings).map(
    (cache: HeadingCache) => cache.heading
);
export const initialHeadingsWithoutFirstLevel = Object.values(
    testHeadingsWithoutFirstLevel
).map((cache: HeadingCache) => cache.heading);
export const initialHeadingsMixed = Object.values(testHeadingsMixed).map(
    (cache: HeadingCache) => cache.heading
);
export const initialHeadingsWithSpecialChars = Object.values(
    testHeadingsWithSpecialChars
).map((cache: HeadingCache) => cache.heading);
...
export function testHandleLinks(fileName: string, content: string, indentation: number) {
    ...
    contentText = contentText.replace(testWikiLinkNoAliasRegex, (_match, refPath: string | number) => {
        // Text including [[wikilink]] -> Text including wikilink
        // OR
        // Text including [[path/to/wikilink]] -> Text including wikilink
        refPath = String(refPath).split('/').pop() ?? String(refPath);
        return refPath;
    });
    alias = alias.replace(testWikiLinkNoAliasRegex, (_match, refPath: string | number) => {
        // [[wikilink]] -> wikilink
        // OR
    ...
    contentText = contentText.replace(testMarkdownLinkRegex, (match, _refAlias) => {
        // Text including [Link](https://www.link.com) -> Text including [Link](https://www.link.com)
        return match;
    });
    alias = alias.replace(testMarkdownLinkRegex, (_match, refAlias: string | number) => {
        // [Link](https://www.link.com) -> Link
        return String(refAlias);
    });

    // Final clean and format for tags and HTML
    contentText = contentText.replace(testTagLinkRegex, (_match, _symbol, tag: string | number) => {
        // Remove any tags
